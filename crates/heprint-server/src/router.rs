...
use heprint_core::{
    BarcodeType, ErrorCode, HeError, ItemType, LineStyle, Orient, PageConfig, PrintItem,
    PrintStyle, Rect, Result,
};
...
pub async fn dispatch(
    ...
    match method {
        // ===== 初始化 =====
        "HE_INIT" | "PRINT_INIT" | "PRINT_INITA" => he_init(params, session),
        "HE_OPEN_TASK" => he_open_task(params, session),
        "HE_CLOSE_TASK" => he_close_task(params, session),
        "HE_LIST_TASKS" => he_list_tasks(session),
        "HE_CLEAR" => he_clear(session),
        "HE_VERSION" => Ok(json!({ "version": HE_VERSION })),

        // ===== 添加内容（支持 task_id 参数）=====
        "HE_ADD_TEXT" | "ADD_PRINT_TEXT" => he_add_text(params, session, None),
        "HE_ADD_HTML" | "HE_ADD_HTM" | "ADD_PRINT_HTM" | "ADD_PRINT_HTML" => {
            he_add_html(params, session, None)
        }
        "HE_ADD_TABLE" | "ADD_PRINT_TABLE" => he_add_table(params, session, None),
        "HE_ADD_IMAGE" | "ADD_PRINT_IMAGE" => he_add_image(params, session, None),
        "HE_ADD_BARCODE" | "ADD_PRINT_BARCODE" => he_add_barcode(params, session, None),
        "HE_ADD_PDF" | "ADD_PRINT_PDF" => he_add_pdf(params, session, None),
        "HE_ADD_LINE" | "ADD_PRINT_LINE" => he_add_line(params, session, None),
        "HE_ADD_RECT" | "ADD_PRINT_RECT" => he_add_rect(params, session, None),

        // ===== 样式 =====
        "HE_SET_STYLE" | "SET_PRINT_STYLE" => he_set_style(params, session),
        "HE_SET_STYLEA" | "SET_PRINT_STYLEA" => he_set_style_a(params, session),

        // ===== 全局参数 =====
        "HE_SET_PAGE" | "SET_PRINT_PAGESIZE" => he_set_page(params, session, None),
        "HE_SET_PRINTER" | "SET_PRINTER_INDEX" => he_set_printer(params, session, None),
        "HE_SET_COPIES" | "SET_PRINT_COPIES" => he_set_copies(params, session, None),
        "HE_SET_OPTION" | "SET_PRINT_MODE" => he_set_option(params, session, None),

        // ===== 执行 =====
        "HE_PRINT" | "PRINT" => he_print(session, false, mgr, None).await,
        "HE_PRINT_SILENT" | "PRINTA" => he_print(session, true, mgr, None).await,
        "HE_PRINT_TASK" => he_print_task(params, session, mgr).await,
        "HE_PREVIEW" | "PREVIEW" => {
            Ok(json!({ "ok": true, "message": "请用 HE_PRINT 后在系统对话框预览" }))
        }
        "HE_NEW_PAGE" | "NEWPAGE" => he_new_page(params, session, None),

        // ===== 打印机查询 =====
        "HE_GET_PRINTERS" | "GET_PRINTER_NAMES" => he_get_printers().await,
        "HE_GET_PRINTER_COUNT" | "GET_PRINTER_COUNT" => he_get_printer_count().await,
        "HE_GET_PRINTER_NAME" | "GET_PRINTER_NAME" => he_get_printer_name(params).await,
        "HE_GET_DEFAULT_PRINTER" | "GET_DEFAULTPRINTER" => he_get_default_printer().await,
        "HE_HAS_PRINTER" | "IS_PRINTER_EXIST" => he_has_printer(params).await,
        "HE_GET_INFO" | "GET_VALUE" => he_get_info(params, mgr),

        // ===== 回调 =====
        "HE_ON_RESULT" => Ok(json!({ "registered": true })),

        // ===== 原生 ESC/POS =====
        "HE_SEND_RAW" | "SEND_PRINT_RAWDATA" => he_send_raw(params).await,
...
fn he_close_task(params: Value, session: &Session) -> Result<Value> {
    let task_id = extract_task_id(&params)
        .ok_or_else(|| HeError::coded(ErrorCode::InvalidParam, "缺少 taskId"))?;
    let mut mgr = session.task_manager.lock();
...
fn he_clear(session: &Session) -> Result<Value> {
    let mut mgr = session.task_manager.lock();
    let current_cleared = mgr.current.take().is_some();
    let task_count = mgr.tasks.len();
    mgr.tasks.clear();
    Ok(json!({
        "ok": true,
        "currentCleared": current_cleared,
        "tasksCleared": task_count
    }))
}
...
fn he_add_table(params: Value, session: &Session, _force_task: Option<String>) -> Result<Value> {
    ...
    .ok_or_else(|| HeError::coded(ErrorCode::TaskNotFound, "请先调用 HE_INIT 或 HE_OPEN_TASK"))?;
    task.push_item(PrintItem::Table {
        bounds,
        style: PrintStyle::default(),
        html,
    });
...
fn he_add_image(params: Value, session: &Session, _force_task: Option<String>) -> Result<Value> {
    ...
    .ok_or_else(|| HeError::coded(ErrorCode::TaskNotFound, "请先调用 HE_INIT 或 HE_OPEN_TASK"))?;
    task.push_item(PrintItem::Image {
        bounds,
        style: PrintStyle::default(),
        src,
    });
...
fn he_add_barcode(params: Value, session: &Session, _force_task: Option<String>) -> Result<Value> {
    ...
    .ok_or_else(|| HeError::coded(ErrorCode::TaskNotFound, "请先调用 HE_INIT 或 HE_OPEN_TASK"))?;
    task.push_item(PrintItem::Barcode {
        bounds,
        style: PrintStyle::default(),
        btype,
        value,
...
fn he_set_style(params: Value, session: &Session) -> Result<Value> {
    ...
    style
        .set(&name, &value)
        .map_err(|e| HeError::coded(ErrorCode::InvalidParam, e))?;
    Ok(json!({ "ok": true }))
}

fn he_set_style_a(params: Value, session: &Session) -> Result<Value> {
    let task_id = extract_task_id(&params);
    let item_index = params
        .get("index")
        .or_else(|| params.get("itemIndex"))
        .and_then(|v| v.as_i64())
        .unwrap_or(0);
    let name = params
        .get("name")
    ...
    let item = if item_index <= 0 {
        ...
        task.items.get_mut((item_index - 1) as usize)
    }
    .ok_or_else(|| {
        HeError::coded(
            ErrorCode::InvalidParam,
            format!("无可设置样式的项: {item_index}"),
        )
    })?;
    ...
    style
        .set(&name, &value)
        .map_err(|e| HeError::coded(ErrorCode::InvalidParam, e))?;
    Ok(json!({ "ok": true, "index": item_index }))
}
...
fn he_set_option(params: Value, session: &Session, _force_task: Option<String>) -> Result<Value> {
    ...
    let key = params
        .get("key")
        .and_then(|v| v.as_str())
        .ok_or_else(|| HeError::coded(ErrorCode::InvalidParam, "缺少 key"))?
        .to_string();
    let value = params.get("value").cloned().unwrap_or(Value::Null);
    let mut mgr = session.task_manager.lock();
    let task = if let Some(tid) = task_id {
...
fn he_new_page(params: Value, session: &Session, _force_task: Option<String>) -> Result<Value> {
    let task_id = extract_task_id(&params);
    let mut mgr = session.task_manager.lock();
    let task = if let Some(tid) = task_id {
        mgr.get_task_mut(&tid)
...
async fn he_print_task(params: Value, session: &Session, mgr: &Arc<PrintManager>) -> Result<Value> {
    let task_id = extract_task_id(&params)
        .ok_or_else(|| HeError::coded(ErrorCode::InvalidParam, "缺少 taskId"))?;
    let silent = params
        .get("silent")
...
async fn he_get_printer_count() -> Result<Value> {
    let printers = tokio::task::spawn_blocking(|| heprint_print::enum_printers())
        .await
        .map_err(|e| HeError::coded(ErrorCode::Unknown, e.to_string()))??;
    Ok(json!({ "count": printers.len() }))
}

async fn he_get_printer_name(params: Value) -> Result<Value> {
    let index = params
        .get("index")
        .or_else(|| params.get("printerIndex"))
        .and_then(|v| v.as_i64())
        .unwrap_or(0);
    let printers = tokio::task::spawn_blocking(|| heprint_print::enum_printers())
        .await
...
async fn he_send_raw(params: Value) -> Result<Value> {
    ...
    let bytes: Vec<u8> = match encoding {
        "hex" => {
            ...
            while let (Some(a), Some(b)) = (chars.next(), chars.next()) {
                let byte = u8::from_str_radix(&format!("{a}{b}"), 16).map_err(|e| {
                    HeError::coded(ErrorCode::InvalidParam, format!("hex 解析失败: {e}"))
                })?;
                out.push(byte);
            }
            out
        ...
        "base64" | _ => {
            ...
            base64::engine::general_purpose::STANDARD
                ...
                .map_err(|e| {
                    HeError::coded(ErrorCode::InvalidParam, format!("base64 解析失败: {e}"))
                })?
        }
    };
    ...
    let result =
        tokio::task::spawn_blocking(move || heprint_print::send_raw_to_printer(&pn, &bytes))
            .await
